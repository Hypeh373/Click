...
def init_db():
    with db_lock:
        ...
        # New settings for Creator hosting and example username
        cursor.execute("INSERT OR IGNORE INTO settings (key, value) VALUES ('creator_hosting_price_usd', '5.0')")
        cursor.execute("INSERT OR IGNORE INTO settings (key, value) VALUES ('creator_example_username', '@your_creator')")
        conn.commit()
...
def get_child_bot_user_count(bot_id, bot_type):
    db_filename_map = {
        ...
        'creator': f"dbs/bot_{bot_id}_creator_data.db"
    }
...
def update_bot_setting(bot_id, setting_name, new_value):
    allowed_settings = [
        ...
        'daily_bonus_clicker', 'daily_bonus_cooldown_clicker', 'ref_bonus_referrer_clicker', 
        'ref_bonus_new_user_clicker', 'withdrawal_min_clicker', 'withdrawal_method_text_clicker', 
        'payments_channel_clicker', 'support_chat_clicker', 'clicker_flyer_api_key', 'clicker_op_enabled',
    ]
...
def start_bot_process(bot_id):
    ...
    try:
        ...
        elif bot_info['bot_type'] == 'creator':
            script_name = CREATOR_BOT_SCRIPT_NAME
        else:
...
def create_main_menu(user_id):
    ...
    if user_id == ADMIN_ID: markup.add(types.KeyboardButton("👑 Админ-панель"))
    return markup
...
def create_admin_menu():
    ...
    markup.add(types.InlineKeyboardButton("🎨 Настройки Креатор", callback_data="admin_creator_manage"))
    markup.add(types.InlineKeyboardButton("💸 Выдать баланс", callback_data="admin_balance_add_start"),
               types.InlineKeyboardButton("🔄 Перезапуск по фильтру", callback_data="admin_restart_filter_start"))
...
def create_bot_type_menu():
    ...
    # Добавляем кнопку Креатор
    creator_price = get_setting('creator_price') or '500.0'
    markup.add(types.InlineKeyboardButton("🎨 Креатор", callback_data="create_bot_creator"))
    # Красивое пояснение о стоимости
    hosting_price = get_setting('creator_hosting_price_usd') or '5.0'
    example_username = get_setting('creator_example_username') or '@your_creator'
    info_text = (
        "\n<b>ℹ️ Важно:</b> Все боты в этом конструкторе — <b>бесплатные</b>,\n"
        f"кроме бота <b>Креатор</b>. Его стоимость: <b>{creator_price} ₽</b>.\n\n"
        "Запуск производится <b>вручную</b>.\n"
        f"Оплата за хостинг бота: <b>{hosting_price}$</b> в месяц.\n"
        f"Пример Креатора: {example_username}.\n"
        "Настройка цены и @username — через админ-меню."
    )
    try:
        # Если вызываем из контекста отправки сообщения, покажем подсказку отдельным сообщением
        pass
    except Exception:
        pass
    return markup
...
def create_bot_actions_menu(bot_id):
    ...
    markup.add(types.InlineKeyboardButton("🗑️ Удалить БД", callback_data=f"db_{bot_id}_delete_confirm"),
    ...
    # Добавляем кнопку для покупки бота Креатор, если это не бот Креатор
    if bot_info['bot_type'] != 'creator':
        creator_price = get_setting('creator_price') or '500.0'
        markup.add(types.InlineKeyboardButton(f"🎨 Купить Креатор ({creator_price} ₽)", callback_data=f"buy_creator_{bot_id}"))
...
def create_dop_zarabotok_menu(bot_id):
    ...
    if bot_info['bot_type'] == 'ref':
        ...
        sub_reward = get_setting('stars_sub_reward') or "1.0" # Используем одну настройку для всех
        text = (f"💰 *Дополнительный заработок*\n\n"
...
def process_state_input(message):
    ...
    # Проверяем отмену для текстовых и медиа сообщений
    if getattr(message, 'text', None) == '❌ Отмена' or getattr(message, 'caption', None) == '❌ Отмена':
    ...
    if action == 'admin_broadcast_get_content':
        # Проверяем, что это не сообщение с отменой
        if getattr(message, 'text', None) == '❌ Отмена' or getattr(message, 'caption', None) == '❌ Отмена':
            return  # Уже обработано выше
            
        bot.delete_message(user_id, state['message_id'])
        
        # Определяем тип контента и сохраняем file_id для медиа
        content_type = getattr(message, 'content_type', 'text')
        data_state = {'action': 'admin_broadcast_get_button', 'content_message_id': message.message_id, 'content_type': content_type}
        
        if content_type == 'photo':
            data_state['photo_id'] = message.photo[-1].file_id
        elif content_type == 'video':
            data_state['video_id'] = message.video.file_id
        elif content_type == 'document':
            data_state['document_id'] = message.document.file_id
            
        set_user_state(user_id, data_state)
    ...
    if action == 'admin_broadcast_get_button':
        ...
        if getattr(message, 'content_type', None) != 'text' or getattr(message, 'text', None) is None:
            bot.send_message(user_id, "❌ Пожалуйста, отправьте текстовое сообщение для настройки кнопки.", reply_markup=create_cancel_markup())
            return
        ...
        try:
            ...
            preview_msg = bot.copy_message(user_id, from_chat_id=user_id, message_id=content_msg_id, reply_markup=button_markup)
    ...
    if action == 'admin_set_new_op_key_admin':
        ...
        if bot_info['bot_type'] == 'ref':
            ...
            update_bot_setting(bot_id, 'clicker_flyer_api_key', new_key)
            update_bot_setting(bot_id, 'clicker_op_enabled', True if new_key else False)
    ...
    if action == 'admin_change_setting':
        ...
        try:
            ...
            elif setting_key == 'creator_price':
                callback_to_return = "admin_creator_manage"
            elif setting_key == 'creator_hosting_price_usd':
                callback_to_return = "admin_creator_manage"
    ...
    if action == 'admin_change_creator_example_username':
        example_username = message.text.strip()
        set_setting('creator_example_username', example_username)
        try: bot.delete_message(user_id, state['message_id'])
        except: pass
        try: bot.delete_message(user_id, message.message_id)
        except: pass
        if user_id in user_states: del user_states[user_id]
        bot.answer_callback_query(state['call_id'], "✅ Настройка обновлена!")
        call_imitation = types.CallbackQuery(id=state['call_id'], from_user=message.from_user, data='admin_creator_manage', chat_instance='private', json_string='')
        fake_message = types.Message(message_id=state['message_id'], from_user=None, date=None, chat=message.chat, content_type='text', options={}, json_string="")
        call_imitation.message = fake_message
        handle_admin_callbacks(call_imitation)
        return
    ...
    if action == 'admin_lists_add_manual':
        ...
        try:
            ...
            # Вернуться в просмотр управления по ID
...
def handle_admin_callbacks(call):
    ...
    if call.data.startswith("admin_lists_") or call.data.startswith("admin_list_"):
        # Guard: disable when feature is off
        try:
            bl_enabled_raw = get_setting('bots_list_feature_enabled')
            bl_enabled = str(bl_enabled_raw).strip() in ('1', 'true', 'True') if bl_enabled_raw is not None else True
        except Exception:
        ...
        if call.data == 'admin_lists_menu':
            ...
            markup.add(types.InlineKeyboardButton("➕ Добавить бота вручную", callback_data="admin_list_add_manual"))
        ...
        if call.data == 'admin_lists_set_min':
            ...
            return
        # Удалено: отдельная страница управления списками. Остается только управление по ID.
        if call.data == 'admin_lists_manage':
            call.data = 'admin_list_byid_start'
            handle_admin_callbacks(call)
            return

        # Режим: управление по введенному ID
            ...
            try:
                if len(parts_local) == 4 and parts_local[3].isdigit():
                    ...
                    if action2 in ("pin", "unpin"):
                        ...
                        if action2 == "pin": pinned.add(bid)
                        else: pinned.discard(bid)
                    ...
                    if action2 in ("hide", "unhide"):
                        hidden = set(json.loads(get_setting('bots_list_hidden') or '[]'))
                        if action2 == "hide": hidden.add(bid)
                        else: hidden.discard(bid)
                    ...
                    if action2 in ("add", "del"):
                        manual = set(json.loads(get_setting('bots_list_manual') or '[]'))
                        if action2 == "add": manual.add(bid)
                        else: manual.discard(bid)
                        set_setting('bots_list_manual', json.dumps(sorted(manual)))
                        bot.answer_callback_query(call.id, "Готово")
                        render_view(bid)
                        return
    ...
    if action == "creator":
        ...
        if sub_action == "manage":
            ...
            example_username = get_setting('creator_example_username') or '@your_creator'
            markup = types.InlineKeyboardMarkup(row_width=1)
            markup.add(types.InlineKeyboardButton(f"💰 Изменить цену Креатор ({creator_price} ₽)", callback_data="admin_creator_set_price"))
            markup.add(types.InlineKeyboardButton(f"💲 Цена хостинга: {hosting_price}$ / мес", callback_data="admin_creator_set_hosting"))
            markup.add(types.InlineKeyboardButton(f"📛 Пример @username: {example_username}", callback_data="admin_creator_set_example"))
            markup.add(types.InlineKeyboardButton("⬅️ Назад", callback_data="admin_back"))
            bot.edit_message_text("🎨 Управление настройками Креатора:", ADMIN_ID, call.message.message_id, reply_markup=markup)
        
        elif sub_action == "set" and parts[3] == "price":
            msg = bot.edit_message_text("Введите новую цену для бота Креатор (например, 500.0):", ADMIN_ID, call.message.message_id,
                                        reply_markup=types.InlineKeyboardMarkup().add(types.InlineKeyboardButton("⬅️ Назад", callback_data="admin_creator_manage")))
            set_user_state(ADMIN_ID, {'action': 'admin_change_setting', 'setting_key': 'creator_price', 'message_id': msg.message_id, 'call_id': call.id, 'message': call.message})
        elif sub_action == "set" and parts[3] == "hosting":
            msg = bot.send_message(ADMIN_ID, "Введите новую цену ХОСТИНГА для Креатора в $/месяц (например, 5.0):", reply_markup=create_cancel_markup())
            set_user_state(ADMIN_ID, {'action': 'admin_change_setting', 'setting_key': 'creator_hosting_price_usd', 'message_id': msg.message_id, 'call_id': call.id, 'message': call.message})
        elif sub_action == "set" and parts[3] == "example":
            msg = bot.send_message(ADMIN_ID, "Введите пример @username (например, @your_creator):", reply_markup=create_cancel_markup())
            set_user_state(ADMIN_ID, {'action': 'admin_change_creator_example_username', 'message_id': msg.message_id, 'call_id': call.id})
        return
    ...
    if action == "back":
        ...
        if sub_action == "start":
            ...
            def run_broadcast_thread():
                ...
                for i, user in enumerate(users_to_send):
                    try:
                        bot.copy_message(user['user_id'], from_chat_id=ADMIN_ID, message_id=preview_message_id, reply_markup=reply_markup_to_send)
                        success_count += 1
        ...
        elif sub_action == "creator":
            users_list = db_execute("SELECT user_id, username FROM users", fetchall=True)
            text = f"<b>👥 Пользователи конструктора ({len(users_list)}):</b>\n\n" + ('\n'.join([f"- ID: <code>{u['user_id']}</code> (@{escape(u['username'] or 'N/A')})" for u in users_list]) or "Список пуст.")
            markup = types.InlineKeyboardMarkup().add(types.InlineKeyboardButton("⬅️ Назад к спискам", callback_data="admin_lists_menu"))
            bot.edit_message_text(text, ADMIN_ID, call.message.message_id, parse_mode="HTML", reply_markup=markup)
        elif sub_action == "set":
        ...
        if sub_action == "manage":
            ...
            markup.add(types.InlineKeyboardButton(f"⭐ Награда за подписку (звёзды/кликер): {stars_reward} ₽", callback_data="admin_op_set_stars_reward"))
            markup.add(types.InlineKeyboardButton("⬅️ Назад", callback_data="admin_back"))
            bot.edit_message_text("⚙️ Управление доходом владельцев ботов от внешних систем (Flyer):", ADMIN_ID, call.message.message_id, reply_markup=markup)
        elif sub_action == "set":
...
def get_bot_flyer_key_for_check(bot_id):
    bot_info = db_execute("SELECT flyer_api_key, stars_flyer_api_key, clicker_flyer_api_key, bot_type FROM bots WHERE id = ?", (bot_id,), fetchone=True)
    if not bot_info:
        return None
    if bot_info['bot_type'] == 'ref':
        ...
        return bot_info['clicker_flyer_api_key']
    return None
...
if __name__ == '__main__':
            ...
            while True:
                try:
                    ...
                    if pending_invoices:
                        ...
                        checked_invoices = await crypto_pay.get_invoices(invoice_ids=",".join(invoice_ids))
    ...
    def handle_start(message):
        ...
        bot.send_message(message.chat.id, "👋 Добро пожаловать! Выберите действие:", reply_markup=create_main_menu(message.from_user.id))
    ...
    def handle_text_buttons(message):
        ...
        if user_id in user_states and getattr(message, 'text', None) and getattr(message, 'text', None) != '❌ Отмена':
            del user_states[user_id]
        
        if getattr(message, 'text', None) == "➕ Создать бота":
            count = get_user_bots_count(user_id)
            if count >= MAX_BOTS_PER_USER and user_id != ADMIN_ID:
                bot.send_message(user_id, f"❌ *Лимит достигнут!* Вы создали {count} из {MAX_BOTS_PER_USER} ботов.", parse_mode="Markdown")
                return
            # Пояснение о стоимости Креатора
            creator_price = get_setting('creator_price') or '500.0'
            info_text = (
                "Выберите тип бота для создания 🧰:\n\n"
                "<i>Все боты — бесплатные, кроме <b>Креатора</b>.</i>\n"
                f"Стоимость Креатора: <b>{creator_price} ₽</b>. Он выдается <b>вручную после оплаты</b>."
            )
            bot.send_message(user_id, info_text, parse_mode="HTML", reply_markup=create_bot_type_menu())
        elif getattr(message, 'text', None) == "📋 Списки ботов":
            try:
                bl_enabled_raw = get_setting('bots_list_feature_enabled')
                bl_enabled = str(bl_enabled_raw).strip() in ('1', 'true', 'True') if bl_enabled_raw is not None else True
            except Exception:
            ...
            for bid, uname, btype, cnt, link in listed[:50]:
                type_icon = "💸" if btype == 'ref' else "⭐" if btype == 'stars' else "🖱" if btype == 'clicker' else "🎨"
            ...
            chat_button = types.InlineKeyboardButton("Чат Администраторов 💬", url=ADMIN_CHAT_LINK) if ADMIN_CHAT_LINK and "your_admin_chat" not in ADMIN_CHAT_LINK else None
            channel_button = types.InlineKeyboardButton("Канал 📢", url=CHANNEL_LINK) if CHANNEL_LINK and "your_channel" not in CHANNEL_LINK else None
        ...
        elif getattr(message, 'text', None) == "👑 Админ-панель" and user_id == ADMIN_ID:
            bot.send_message(user_id, "👑 Админ-меню", reply_markup=create_admin_menu())
        ...
        try:
                ...
                if action == 'toggle':
                    ...
                    if parts[3] == 'pay':
                        ...
                        async def create_invoice_async():
                            try:
                                invoice = await crypto_pay.create_invoice(asset='USDT', amount=vip_price, fiat='RUB', payload=f"vip_{bot_id}")
                                if invoice:
                    ...
                    async def check_single_invoice():
                        invoices = await crypto_pay.get_invoices(invoice_ids=str(invoice_id_to_check))
            ...
            if call.data.startswith('buy_creator_'):
                ...
                text = (f"🎨 <b>Покупка бота Креатор</b>\n\n"
                        ...
                        f"<b>Важно:</b> Запуск производится вручную. Хостинг: <b>{hosting_price}$</b>/месяц.\n"
                        f"Пример креатора: {example_username}. Настройка цены и @username — через админ-меню.\n\n"
                        f"Выберите способ оплаты:")
                markup = types.InlineKeyboardMarkup(row_width=1)
                if crypto_pay:
                ...
                if action == 'crypto' and parts[2] == 'pay':
                    ...
                    async def create_creatornew_invoice_async():
                        try:
                            ...
                            invoice = await crypto_pay.create_invoice(asset='USDT', amount=creator_price, fiat='RUB', payload=payload)
                            if invoice:
                                db_execute("INSERT INTO crypto_payments (invoice_id, bot_id, user_id, amount, status) VALUES (?, ?, ?, ?, 'pending')",
                ...
                if len(parts) >= 4 and parts[1].isdigit():
                    ...
                    if action == 'crypto' and parts[3] == 'pay':
                        ...
                        async def create_creator_invoice_async():
                            try:
                                invoice = await crypto_pay.create_invoice(asset='USDT', amount=creator_price, fiat='RUB', payload=f"creator_{bot_id}")
                                if invoice:
                                    db_execute("INSERT INTO crypto_payments (invoice_id, bot_id, user_id, amount, status) VALUES (?, ?, ?, ?, 'pending')",
            ...
            if call.data == "create_bot_creator":
                # Предложить оплату для создания Креатора через Crypto Pay
                try:
                    creator_price = float(get_setting('creator_price') or 500.0)
                except Exception:
                    creator_price = 500.0
                ...
                markup = types.InlineKeyboardMarkup(row_width=1)
                if crypto_pay:
                    markup.add(types.InlineKeyboardButton("₽ Crypto Bot", callback_data=f"creatornew_crypto_pay"))
                markup.add(types.InlineKeyboardButton("👤 Другой способ", callback_data=f"creatornew_other_payment"))
                markup.add(types.InlineKeyboardButton("⬅️ Назад", callback_data="back_to_bots_list"))
                bot.edit_message_text(text, user_id, call.message.message_id, parse_mode="HTML", reply_markup=markup); return
            ...
            elif action == 'db' and data[2] == 'delete':
                if data[3] == 'confirm':
                    markup = types.InlineKeyboardMarkup().add(types.InlineKeyboardButton("❗️ ДА, УДАЛИТЬ БД ❗️", callback_data=f"db_{bot_id}_delete_final"), types.InlineKeyboardButton("⬅️ Отмена", callback_data=f"actions_{bot_id}"))
                    bot.edit_message_text("Вы уверены, что хотите удалить базу данных этого бота? ВСЕ ЕГО ПОЛЬЗОВАТЕЛИ БУДУТ СТЕРТЫ НАВСЕГДА.", user_id, call.message.message_id, reply_markup=markup)
                elif data[3] == 'final':
                    try: 
                        bot_info = get_bot_by_id(bot_id)
                        db_filename_map = {
                        ...
                        if db_filename:
                            os.remove(db_filename)
                        bot.answer_callback_query(call.id, f"✅ База данных для бота #{bot_id} успешно удалена.", show_alert=True)
                    except FileNotFoundError: 
                        bot.answer_callback_query(call.id, f"❌ База данных для бота #{bot_id} не найдена.", show_alert=True)
                    call.data = f"actions_{bot_id}"; handle_callback_query(call)
